...
void TAN_IM6::train(const instance &inst)
{
    ...
    for (CatValue y_ = 0; y_ < noClasses_; y_++){
        std::vector<double>classDist_cly(this->noClasses_,1.0);
        for (CatValue y = 0; y < noClasses_; y++)
        {
            classDist_cly[y] = xxyDist_.xyCounts.p(y);
        }

        for (unsigned int x1 = 0; x1 < noCatAtts_; x1++)
        {
            const CategoricalAttribute parent = parents_y[y_][x1];

            if (parent == NOPARENT)
            {
                for (CatValue y = 0; y < noClasses_; y++)
                {
                    classDist_cly[y] *= xxyDist_.xyCounts.p(x1, inst.getCatVal(x1), y);
                }
            }
            else
            {
                for (CatValue y = 0; y < noClasses_; y++)
                {
                    classDist_cly[y] *= xxyDist_.p(x1, inst.getCatVal(x1), parent,
    ...
    for (unsigned int x1 = 0; x1 < noCatAtts_; x1++)
    {
        const CategoricalAttribute parent = parents_y[cur_best_y][x1];
...
void TAN_IM6::classify(const instance &inst, std::vector<double> &classDist)
{
    ...
    classify_local(inst,parents_y[model_selected],classDist);
