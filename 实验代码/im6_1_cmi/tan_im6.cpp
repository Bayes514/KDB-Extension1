...
void TAN_IM6::remodel(std::vector<unsigned int> &parrent,crosstab<float> &cmi){
    for(int i = 0;i<this->noCatAtts_;i++){
        if(parrent[i] == this->NOPARENT){
            continue;
        }
        int p  = parrent[i];
        double sum = 0.0;
        for(int j = 0 ;j<this->noCatAtts_;j++){
            if ( p == j)
                continue;
            sum+= cmi[p][j];
        }
        if (cmi[p][i] < (this->threshold/100.0) * sum){
            parrent[i] = this->NOPARENT;
